...
4. Опционально, если скрипт об этом "попросили", он сохранит данные и в базу данных (PostgreSQL).
...
Следует немного разобрать пример:

Номер маршрута, как видно из примера, может быть как и простым (511, н5), так и длинной сложной строкой (Южные Ворота - Братиславская), обычно для маршруток.
Для рейсов у которых есть расчетное время прибытия на остановку, частота обычно не указывается, и наоборот.

Расчетное время прибытия специально разделено на "ближайшее" и "последующие", в ближайшем будет всегда всего одно число, тогда как в последнющих может быть несколько чисел разделенных пробелом, если, например, к остановке уже подхезжает несколько автобусов одного маршрута.

Наконец, для некоторых автобусов, не часто выполняющих свои рейсы, иногда будет доступна еще и информация о "точном отправлении", особенно если станция конечная (автобус 280 в примере).
...
Здесь хорошо видно несколько прибытия трамвая 50 и 45 к данной остановке. Например читать строку `('3', '45', 'Трамвай', '', '12', '13 23', '')` следует следующим образом:
...
3 - вывод сообщений об ошибках, предупреждений и информациионных сообщений (INFO) \
